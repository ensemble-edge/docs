    ...
    steps:
      ...
      - name: Auto-version changed components
        run: |
          ...
          CHANGED=$(ensemble edgit discover scan --changed --output simple | awk '{print $2}' | sort -u)

          # Auto-increment versions for each changed component
          for component in $CHANGED; do
            LATEST=$(ensemble edgit tag list $component 2>/dev/null | tail -1 || echo "v0.0.0")
            NEW_VERSION=$(ensemble edgit version bump $LATEST patch)
            ensemble edgit tag create $component $NEW_VERSION
      ...
      - name: Version Components
        run: |
          ...
          for file in $CHANGED_FILES; do
            COMPONENT=$(ensemble edgit discover detect $file --name-only)
            if [ ! -z "$COMPONENT" ]; then
              LATEST=$(ensemble edgit tag list $COMPONENT | tail -1 || echo "v0.0.0")
              NEW=$(ensemble edgit version bump $LATEST patch)
              ensemble edgit tag create $COMPONENT $NEW
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to Staging
        run: |
          # Get all components
          COMPONENTS=$(ensemble edgit components list --format json | jq -r '.[] | .name')

          # Deploy latest version of each to staging
          for component in $COMPONENTS; do
            LATEST=$(ensemble edgit tag list $component | tail -1)
            ensemble edgit deploy set $component $LATEST --to staging
          done
    ...
    steps:
      ...
      - name: Promote to Production
        run: |
          ...
          for component in $COMPONENTS; do
            # Get staging version
            STAGING_VERSION=$(ensemble edgit deploy get $component --from staging)

            # Deploy to production
            ensemble edgit deploy set $component $STAGING_VERSION --to prod
...
jobs:
  version:
    ...
    steps:
      ...
      - run:
          name: Install Ensemble CLI
          command: npm install -g @ensemble-edge/ensemble
      - run:
          name: Version Components
          command: |
...
if echo "$COMMITS" | grep -q "BREAKING CHANGE\|breaking:"; then
  # Major version bump
  NEW=$(ensemble edgit version bump $LATEST major)
elif echo "$COMMITS" | grep -q "feat:"; then
  # Minor version bump
  NEW=$(ensemble edgit version bump $LATEST minor)
else
  # Patch version bump
  NEW=$(ensemble edgit version bump $LATEST patch)
fi
    ...
    steps:
      ...
      - name: Export Version Data
        run: |
          ...
          ensemble edgit components list --format json > versions.json
