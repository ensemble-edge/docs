...
Edgit uses **type-specific namespaced** Git tags. Each component type has its own namespace:

```bash
# Prompts use prompts/ namespace
prompts/extraction-prompt/v1.0.0
prompts/extraction-prompt/v1.1.0
prompts/extraction-prompt/prod      # Deployment tag

# Schemas use schemas/ namespace
schemas/user-schema/v1.0.0
schemas/user-schema/staging         # Deployment tag

# Configs use configs/ namespace
configs/api-config/v1.0.0

# Scripts use scripts/ namespace
scripts/transform/v1.0.0

# Queries use queries/ namespace
queries/analytics-query/v1.0.0

# Templates use templates/ namespace
templates/email-header/v1.0.0
```

**Type-to-Namespace Mapping:**

| Component Type | Tag Namespace |
|----------------|---------------|
| `prompt` | `prompts/` |
| `schema` | `schemas/` |
| `config` | `configs/` |
...
View them:
```bash
git tag | grep "prompts/extraction-prompt"
```
...
Agents use the `agents/` namespace:
...
**Why type-specific namespaces?**
- Clear distinction between component types
- No naming collisions (you can have a prompt and agent with the same name)
- Better organization in Git
- Aligns with Conductor's KV storage namespaces
...
**Ensembles** are workflow definitions that orchestrate agents. They can be:
...
<Tabs>
  <Tab title="TypeScript">
```bash
# Register a TypeScript ensemble
...
edgit tag create data-pipeline v1.1.0 --type=ensemble
edgit tag create data-pipeline v2.0.0 --type=ensemble
```
  </Tab>
  <Tab title="YAML">
```bash
# Register a YAML ensemble
edgit components add ensemble scraping-workflow ensembles/scraping.yaml

# Create versions
edgit tag create scraping-workflow v1.0.0 --type=ensemble
edgit tag create scraping-workflow v1.5.0 --type=ensemble
```
  </Tab>
</Tabs>

### Git Tag Format for Ensembles

Ensembles use the `ensembles/` namespace:

```bash
ensembles/data-pipeline/v1.0.0
ensembles/data-pipeline/v1.1.0
ensembles/data-pipeline/v2.0.0
ensembles/data-pipeline/prod      # Deployment tag
ensembles/data-pipeline/staging   # Deployment tag
```

This follows the same type-specific namespace pattern as all other component types.
...
```bash
# Error: prompts/my-prompt/v1.0.0 not found

# Check if tag exists (note: use the correct namespace)
git tag | grep "prompts/my-prompt"

# List all versions
edgit tag list my-prompt

# Check your component type - namespace depends on type
edgit components list
# prompts/ for prompts, schemas/ for schemas, agents/ for agents, etc.
```
...
# Check existing tag (use correct namespace)
git show prompts/my-prompt/v1.0.0
