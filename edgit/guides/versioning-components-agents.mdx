...
const companyIntel = createEnsemble({
  name: 'company-intel',
  steps: [
    versionedAgent('analyzer', '1.0.0', {
      config: { model: 'claude-sonnet-4' }
    }).toFlowStep(),
    versionedAgent('scorer', '2.1.0').toFlowStep()
  ]
})
...
Ensembles use their own namespace:
...
**Why separate namespaces?**
- Clear distinction between components, agents, and ensembles
- No naming collisions
- Better organization in Git
...
import { createEnsemble, step, parallel, branch } from '@anthropic/conductor'
