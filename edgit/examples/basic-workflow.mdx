...
- Edgit installed globally: `npm install -g @ensemble-edge/edgit`
...
List all components and view details with multiple output formats:
...
📁 function:
   📦 email-validator: v1.1.0 (2 versions) [staging,production]
```

### Tree Format with Version History

```bash
# Visual tree showing all versions and deployment indicators
...
└─ function (1 components)
   └─ email-validator
      ├─ v1.1.0 (abc5678, 2025-11-01) ← production
      └─ v1.0.0 (abc1234, 2025-11-01) ← staging
```
...
[
  {
    ...
    "deploymentTags": ["staging", "production"],
    "versionCount": 2
  }
]
```
...
```bash
# View detailed history for a specific component
...
# 6. Check status
edgit components list --format tree  # Visual overview
edgit status email-validator         # Detailed component info
```
