...
View comprehensive Edgit status including initialization state, tracked components, recent versions, and deployments:

```bash
ensemble edgit status [options]
```
...
<Note>
The status command is served by the Ensemble CLI for a rich, consistent experience across all products. Other edgit commands pass through to the standalone edgit CLI.
</Note>
