...
- **Template rendering**: Liquid templates with KV storage support
...
Email member supports template engines for rendering email content:

```yaml
config:
  templateEngine: liquid  # 'liquid' (default), 'simple'
  template: kv://templates/email/welcome
```

### Liquid (Default, Recommended)

Shopify template language with powerful filters. Edge-compatible and industry standard:

```html
<h1>Welcome, {{ name | capitalize }}!</h1>
<p>Order total: {{ price | money_with_currency: "USD" }}</p>
<p>Placed {{ createdAt | time_ago }}</p>
{% if isPremium %}
  <div class="premium-offer">Special offer for you!</div>
{% endif %}
```

**Why Liquid?**
- Edge-compatible - no CSP issues
- Industry standard (Shopify, Jekyll)
- Real-time rendering at the edge
- Rich filter library for common tasks

### Simple

Basic `{{variable}}` substitution. Fast and lightweight for simple templates:
...
**Note about Handlebars**: Previously supported Handlebars uses `new Function()` which is blocked by Cloudflare Workers CSP. Use Liquid instead for edge-compatible templates.
