...
<Note>
  **Why "Members" instead of "Agents"?**

  While members ARE agents that perform tasks, we use this term to:
  ...
  Throughout this documentation, you'll see "member" and "agent" used interchangeably—they mean the same thing in Conductor. Members are the executable units that perform work within your workflows.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="HTML" icon="window" href="/conductor/member-types/html">
    HTML rendering with templates and cookie management
  </Card>

  <Card title="PDF" icon="file-pdf" href="/conductor/member-types/pdf">
    PDF generation from HTML with R2 storage
  </Card>

  <Card title="Page" icon="browser" href="/conductor/member-types/page">
    Full-stack pages with SSR, dynamic routing, and htmx
  </Card>
</CardGroup>
...
**Note**: The `member` field references member definitions, not versioned components. To use versioned components (prompts, templates, etc.) within members, reference them in the member's configuration:
...
flow:
  - name: analyze
    type: Think
    config:
      prompt: prompts/analyst@v1.2.0    # Versioned prompt via Edgit

  - name: send-email
    type: Email
    config:
      template: templates/welcome@v2.0.0  # Versioned template via Edgit
```

## Creating Custom Members

Export an async function that receives input and returns output:

```typescript
// members/your-member/index.ts
export default async function yourMember({ input, env, ctx }) {
  // Your logic here
