    ...
    steps:
      ...
      - name: Generate commit message suggestion
        ...
        run: |
          ...
          # Post as PR comment
          gh pr comment ${{ github.event.pull_request.number }} --body "### 🤖 Suggested Commit Message\n\n\`\`\`\n$message\n\`\`\`"
        env:
...
before_script:
  - npm install -g @ensemble-edge/edgit@$EDGIT_VERSION
...
pipeline {
    ...
    stages {
        stage('Setup') {
            steps {
                sh 'npm install -g @ensemble-edge/edgit'
            }
        }
