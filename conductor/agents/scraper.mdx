---
title: "scraper Agent"
description: "Production-grade web scraping. Handles failures, timeouts, and rate limits."
---
...
agents:
  ...
  - name: handle-error
    ...
    input:
      ...
      error_status: ${scrape.error.status}
      retryable: ${scrape.error.retryable}
...
export default function handleScrapeError(context: AgentExecutionContext) {
  const { error_message, error_status, retryable } = context.input
...
export default function combineScrapedContent(context: AgentExecutionContext) {
  ...
  return {
    content: scraped_results.map((r: any) => r.text).join('\n\n')
  }
}
```
