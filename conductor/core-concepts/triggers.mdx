---
title: "Triggers"
description: "Invoke ensembles via webhooks, MCP tools, email, queues, cron schedules, and startup using the unified trigger system"
---
...
To **set cookies** in responses, use the [cookies operation](/conductor/operations/cookies):
...
The cookies operation integrates with [Location Context](/conductor/core-concepts/location-context) for GDPR/CCPA consent-aware cookie management.
...
<Warning>
**Keep startup triggers fast (<5 seconds).** Cloudflare Workers have a 30-second initialization timeout. While startup triggers run non-blocking via `waitUntil()`, slow triggers delay background task completion.
</Warning>
