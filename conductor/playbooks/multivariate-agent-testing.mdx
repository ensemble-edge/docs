...
description: "Test combinations of models, prompts, temperatures, and agents. Find the optimal configuration faster by testing multiple variables simultaneously instead of just 2 variants."
---
...
agents:
  ...
  - name: pattern-sequential
    ...
    operation: code
    config:
      script: scripts/execute-agents-sequentially
    input:
...
```typescript
// scripts/execute-agents-sequentially.ts
import type { AgentExecutionContext } from '@ensemble-edge/conductor'

export default async function executeAgentsSequentially(context: AgentExecutionContext) {
  const { agents, initialInput } = context.input

  // Execute agents sequentially
  let currentInput = initialInput
  for (const agentName of agents) {
    const result = await executeAgent(agentName, currentInput)
...
agents:
  ...
  - name: select-variant
    operation: code
    config:
      script: scripts/select-variant
    input:
      allocations: ${get-allocations.output}
      userId: ${input.user_id}
```

```typescript
// scripts/select-variant.ts
import type { AgentExecutionContext } from '@ensemble-edge/conductor'

export default function selectVariant(context: AgentExecutionContext) {
  const { allocations, userId } = context.input
