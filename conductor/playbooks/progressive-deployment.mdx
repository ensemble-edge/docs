...
```bash
# Tag v2.0.0 for green (no traffic yet)
edgit tag set my-prompt green v2.0.0
...
agents:
  ...
  - name: combine-data
    operation: code
    config:
      script: scripts/combine-user-data
    input:
      userData: ${fetch-user-data.output.body}
      ordersData: ${fetch-order-history.output.body}
      preferencesData: ${fetch-preferences.output.body}
...
```typescript
// scripts/combine-user-data.ts
import type { AgentExecutionContext } from '@ensemble-edge/conductor'

export default function combineUserData(context: AgentExecutionContext) {
  const { userData, ordersData, preferencesData } = context.input
...
agents:
  ...
  - name: handle-results
    operation: code
    config:
      script: scripts/handle-task-results
    input:
      criticalSuccess: ${critical-task.success}
      optionalSuccess: ${optional-task.success}
      anotherSuccess: ${another-task.success}
...
```typescript
// scripts/handle-task-results.ts
import type { AgentExecutionContext } from '@ensemble-edge/conductor'

export default function handleTaskResults(context: AgentExecutionContext) {
  const { criticalSuccess, optionalSuccess, anotherSuccess } = context.input

  const failedTasks = [
    !criticalSuccess ? 'critical-task' : null,
    !optionalSuccess ? 'optional-task' : null,
    !anotherSuccess ? 'another-task' : null
  ].filter(Boolean)

  return {
    critical: criticalSuccess,
    optional: optionalSuccess,
    another: anotherSuccess,
    failedTasks
  }
}
```
...
agents:
  ...
  - parallel:
      - name: validate-cart
        operation: code
        config:
          script: scripts/validate-cart
        input:
          cartItems: ${input.cart.items}
  ...
  - name: check-all-valid
    operation: code
    config:
      script: scripts/check-all-validations
    input:
      cartValid: ${validate-cart.output.valid}
      paymentStatus: ${validate-payment.output.status}
      shippingValid: ${validate-shipping.output.valid}
      inventoryCount: ${check-inventory.output.length}
      expectedItemsCount: ${input.cart.items.length}
...
```typescript
// scripts/validate-cart.ts
import type { AgentExecutionContext } from '@ensemble-edge/conductor'

export default function validateCart(context: AgentExecutionContext) {
  const { cartItems } = context.input

  // Validate cart items
  return { valid: cartItems.length > 0 }
}
```

```typescript
// scripts/check-all-validations.ts
import type { AgentExecutionContext } from '@ensemble-edge/conductor'

export default function checkAllValidations(context: AgentExecutionContext) {
  const { cartValid, paymentStatus, shippingValid, inventoryCount, expectedItemsCount } = context.input

  return {
    allValid: cartValid &&
              paymentStatus === 'valid' &&
              shippingValid &&
              inventoryCount === expectedItemsCount
  }
}
```
