...
- member: render-page
  ...
  template:
    inline: |
      {{> template://components/header}}  # Component cached for 1 hour
      <main>{{content}}</main>
      {{> template://components/footer}}  # Component cached for 1 hour
  data:
    content: "Page content here"
```

Components loaded via `template://` URIs are automatically cached. See [Component Caching](#component-caching) below.
...
const footer = await componentLoader.load('template://components/footer@v1.0.0', {
  cache: { ttl: 86400 }
});

// Cache for 5 minutes (dynamic content)
const news = await componentLoader.load('template://components/news@latest', {
  cache: { ttl: 300 }
});

// Bypass cache (force fresh load)
const liveData = await componentLoader.load('template://components/live-data@latest', {
  cache: { bypass: true }
});
```
...
conductor:cache:components:prompt://analyze-company@v2.0.0
conductor:cache:components:form://contact@latest
```

The cache key includes:
- **Prefix**: `conductor:cache:components:`
- **Protocol**: `template://`, `prompt://`, `query://`, etc.
- **Path**: Component path
...
await componentLoader.load('template://header@v1.0.0');
await componentLoader.load('template://header@v2.0.0');
```
...
await componentLoader.load('template://layouts/main@v1.0.0', {
...
await componentLoader.load('template://components/stock-ticker@latest', {
...
await componentLoader.load('template://components/debug@latest', {
  cache: { bypass: true }
});
```
  ...
  - member: render-dashboard
    ...
    template:
      inline: |
        ...
        <div class="metrics">
          {{> template://components/metrics@latest}}
        </div>
...
```typescript
const metrics = await componentLoader.load('template://components/metrics@latest', {
