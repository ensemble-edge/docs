...
Email member supports four template engines for rendering email content:

```yaml
config:
  templateEngine: handlebars  # 'simple' (default), 'handlebars', 'liquid', or 'mjml'
...
```html
<h1>{{capitalize greeting}}, {{name}}!</h1>
{{#if isPremium}}
...
Responsive email framework (compiles to HTML):
  ...
  <mj-body>
    <mj-section>
      <mj-column>
        ...
        <mj-button href="{{confirmUrl}}">Confirm Email</mj-button>
      </mj-column>
    </mj-section>
  </mj-body>
</mjml>
...
**Note**: MJML may not work in all Cloudflare Workers environments due to Node.js dependencies.
