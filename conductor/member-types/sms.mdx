...
- **Template rendering**: Liquid templates for dynamic content
...
SMS member supports template engines for rendering message content:
...
Shopify template language with powerful filters. Edge-compatible and perfect for dynamic content:
...
input:
  body: |
    ...
    {% endif %}
    Balance: {{ balance | money_with_currency: "USD" }}
```
...
- Rich filter library for formatting
