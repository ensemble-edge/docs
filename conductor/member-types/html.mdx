...
The HTML member supports template engines optimized for edge rendering. Specify your choice in the config:
...
Shopify's template language with powerful filters. Edge-compatible and perfect for modern applications:

```yaml
config:
  templateEngine: liquid  # default, can be omitted
  template:
    inline: |
...
- Rich filter library (`money`, `phone`, `pluralize`, `excerpt`, `strip_html`, `highlight`, `file_size`, `time_ago`, plus all standard Liquid filters)

### Simple Engine

Built-in lightweight engine with `{{variable}}` syntax. Fast and minimal for basic templates:
...
**Note about Handlebars**: Previously supported Handlebars uses `new Function()` which is blocked by Cloudflare Workers CSP. Use Liquid instead for edge-compatible templates with advanced features like conditionals, loops, and filters.
      ...
      template:
        inline: |
          ...
          <body>
            ...
            <main>
              <h1>{{pageTitle}}</h1>
              {% include 'templates/components/content-card'
                title: "Welcome",
