...
<Note type="success">
**Auto-Discovery Available**: Pages are automatically discovered at build time - no manual imports needed! See the [Auto-Discovery Guide](/conductor/guides/auto-discovery) for the modern approach.
</Note>

### Modern Approach (Recommended)

With auto-discovery, pages are automatically imported and registered:

```typescript
import { PageRouter, PageMember } from '@ensemble-edge/conductor';
import { parse as parseYAML } from 'yaml';

// Pages automatically discovered via virtual module
...
// Convert discovered pages to pagesMap
const pagesMap = new Map(
  discoveredPages.map((page) => {
    const config = parseYAML(page.config);
...
**Benefits:**
- ✅ No manual imports - just create pages in `pages/` directory
- ✅ Automatic updates when pages are added/removed
- ✅ Hot Module Replacement during development
- ✅ 3-33x faster builds with Vite + Rolldown

### Legacy Approach (Manual)

<details>
<summary>Click to see manual import approach (not recommended)</summary>

```typescript
import { PageRouter } from '@ensemble-edge/conductor';
import { PageMember } from '@ensemble-edge/conductor/members/page';

// Import page configs manually (tedious!)
...
- ❌ Manual import for every new page
