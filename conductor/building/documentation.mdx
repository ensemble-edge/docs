...
route:
  path: /docs           # Where docs are served
  methods: [GET]
  auth:
    requirement: public
  priority: 50
...
Control where and how docs are served:
...
route:
  ...
  auth:
    requirement: public # public | authenticated | admin
  priority: 50          # Route priority (default: 50)
```
...
Control visibility in `docs.yaml`:
...
Settings in `docs/docs.yaml` override global configuration.
...
2. **Filter endpoints** with `include`/`exclude`
3. **Separate docs** for different audiences
