...
export default async function greeter(
  ...
  // Access location for personalized greeting
  const timeOfDay = ctx.location?.getTimeOfDay() ?? 'day'
  const greetings = {
  ...
  return {
    greeting: `${greetings[timeOfDay]}, ${input.name}!`
  }
}
```
  </Tab>
...
- Access [location](/conductor/core-concepts/location-context) and [edge](/conductor/core-concepts/edge-context) context for geo-aware behavior
...
9. **Use Location Context** - Leverage [location](/conductor/core-concepts/location-context) for localization and compliance
10. **Consider Edge Context** - Use [edge](/conductor/core-concepts/edge-context) for security decisions
