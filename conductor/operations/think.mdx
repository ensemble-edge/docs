...
description: "AI reasoning with language models"
---
...
The `think` operation is your gateway to AI-powered intelligence. Use it for any task involving natural language understanding, content generation, or complex reasoning.
  ...
  - name: consensus
    ...
    config:
      code: |
        ...
        return { answer: mostCommon(answers) };
```
...
# Access output
outputs:
  tagline: ${generate-text.output}  # String response
```
...
# Parse JSON in next operation
  - name: use-data
    operation: code
    config:
      code: |
        const data = JSON.parse('${extract-json.output}');
...
<CardGroup cols={2}>
  ...
  <Card title="Pre-built Agents" icon="layer-group" href="/conductor/agents/overview">
    Agents using think operation
  </Card>
  <Card title="Building Patterns" icon="hammer" href="/conductor/building/patterns">
