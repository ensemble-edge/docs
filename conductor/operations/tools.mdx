...
agents:
  ...
  - name: check-result
    ...
    config:
      code: |
        if (${call-tool.output.isError}) {
          throw new Error('Tool returned error: ' + ${call-tool.output.content});
        }
        return ${call-tool.output.content};
```
...
- name: verify
  ...
  config:
    code: |
      if (${tool-call.failed} || ${tool-call.output.isError}) {
        throw new Error('Tool failed');
      }
      return ${tool-call.output.content};
```
