...
agents:
  ...
  - name: logger
    ...
    config:
      code: |
        const greeting = "${greeter.output.message}";
...
This provides:
- `POST /api/v1/execute` - Execute any ensemble or agent
- `GET /api/v1/ensembles` - List all ensembles (coming soon)
- `GET /api/v1/agents` - List all agents
- Automatic webhook and cron trigger handling

### Execute Request Format

Execute an ensemble via the unified API:
...
Execute an agent via the same API:
...
agents:
  ...
  - name: aggregate
    ...
    config:
      code: |
        return {
          safe: !${check-spam.output.spam} &&
                !${check-hate.output.hate} &&
                !${check-explicit.output.explicit}
        };
...
adWords = badWords.some(w => text.includes(w));
...
agents:
  ...
  - name: fallback-handler
    ...
    config:
      code: |
        console.error("Operation failed:", "${risky-operation.error}");
...
<AccordionGroup>
  <Accordion title="Agent not found">
    ...
    **Fix**: Ensure agent is registered or exists in `agents/` directory:
    ```bash
    npm run build  # Rebuild to discover agents
    ```
  </Accordion>
