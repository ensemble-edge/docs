...
5. **Done!** Your agent is now available at `/api/v1/execute`
...
import greetConfig from '../agents/greeter/agent.yaml'
import greetFunction from '../agents/greeter'
...
import greetFunction from './agents/greet/index.ts'
// ... 50 more imports ...
...
const dataPipeline = createEnsemble('data-pipeline')
  .addStep(
    step('fetch')
      .agent('fetcher')  // References agents/fetcher/agent.yaml
      .input({ url: '${input.sourceUrl}' })
  )
  .addStep(
    step('process')
      .agent('data-processor')  // References agents/data-processor/agent.yaml
...
conductor validate agents/my-agent/agent.yaml
