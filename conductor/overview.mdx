...
Conductor isn't just for backend workflows. It also includes **Pages** (Hono-based routing) for building server-rendered web applications:

**Pages Features:**
- **Hono routing** - Battle-tested, fast HTTP router with middleware support
- **Conductor operations** - Use any operation in `beforeRender` (Payload CMS, Attio, HTTP, Think, etc.)
- **Hono middleware** - Full Hono middleware ecosystem (CORS, compression, JWT, etc.)
- **Template engines** - Handlebars, Liquid, or Simple (same as Email/SMS)
- **Content negotiation** - HTML or JSON based on Accept header
- **Auto-discovery** - Pages loaded from `pages/` directory
...
[Learn more about Pages ](/conductor/core-concepts/pages)
