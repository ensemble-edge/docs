...
Conductor supports four template engines for rendering HTML, Email, and SMS content:

### Simple (Default)

Basic variable substitution with `{{variable}}` syntax. Fast and lightweight, perfect for simple templates:
...
Full-featured template engine with helpers, conditionals, loops, and partials. Best for complex templates:
...
**Built-in helpers**: `eq`, `ne`, `lt`, `gt`, `and`, `or`, `upper`, `lower`, `capitalize`, `truncate`, `formatDate`, `formatCurrency`, and more.

### Liquid

Shopify's template language with powerful filters. Ideal for e-commerce and content-heavy applications:
...
**Built-in filters**: `money`, `phone`, `pluralize`, `excerpt`, `strip_html`, `highlight`, `file_size`, `time_ago`, and all standard Liquid filters.

### MJML

Responsive email framework that generates cross-client compatible HTML emails:
...
**Note**: MJML combines with Handlebars for dynamic content (use `{{variables}}` inside MJML tags). MJML has Node.js dependencies and may not work in all Cloudflare Workers environments.
...
**Engine Availability:**
- **HTML & Email**: `simple` (default), `handlebars`, `liquid`, `mjml`
- **SMS**: `simple` (default), `handlebars`, `liquid`

## Template Types

### Email Templates

HTML emails with your chosen template engine syntax:
